#!/bin/bash
    ...
    echo "[DEBUG] Processing small_chunk9a dataset"
    echo "[DEBUG] Reading CSV file: 9a"
    echo "[DEBUG] File size: $(wc -c < 9a) bytes"
    ...
    cat << EOF > metrics_small_chunk9a.json
{
    "process": "CONVERT_CSV_TO_JSON",
    "dataset_size": "small_chunk9a",
    "metrics": {
        "input_size_bytes": $(wc -c < 9a),
