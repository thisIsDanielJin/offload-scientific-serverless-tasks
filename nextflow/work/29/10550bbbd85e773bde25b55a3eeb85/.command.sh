#!/bin/bash
    ...
    echo "[DEBUG] Reading CSV file: 0e863ec8b2e4e50ca600295e9511c7"
    echo "[DEBUG] File size: $(wc -c < "0e863ec8b2e4e50ca600295e9511c7") bytes"
    echo "[DEBUG] First few lines of CSV:"
    head -n 3 "0e863ec8b2e4e50ca600295e9511c7"

    start_time=$(date +%s%N)

    # Read CSV content without escaping newlines
    csv_content=$(cat "0e863ec8b2e4e50ca600295e9511c7")
    ...
    cat << EOF > metrics_small.json
{
    ...
    "dataset_size": "small",
    "metrics": {
        "input_size_bytes": $(wc -c < "0e863ec8b2e4e50ca600295e9511c7"),
