#!/bin/bash
    ...
    echo "[DEBUG] Processing small_chunkd3 dataset"
    echo "[DEBUG] Reading CSV file: d3"
    echo "[DEBUG] File size: $(wc -c < d3) bytes"
    ...
    cat << EOF > metrics_small_chunkd3.json
{
    "process": "CONVERT_CSV_TO_JSON",
    "dataset_size": "small_chunkd3",
    "metrics": {
        "input_size_bytes": $(wc -c < d3),
